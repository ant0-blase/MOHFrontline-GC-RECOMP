...
static bool instruction_uses_fallback(const PPCInst* inst) {
    /* All decoded cache-control and SPR operations have native C helpers.
     * Keep interpreter fallback exclusively for truly unknown instructions. */
    return inst->op == PPC_OP_UNKNOWN;
}
