...
// CPU ABI v4 adds persistent native scheduler budgets at the end of the
// shared generated-code prefix. Keep their order synchronized with GXRuntime
// and the ModernGekko chassis contract.
#define DOLRECOMP_CPU_ABI_VERSION 4u
...
struct CPUState {
    ...
    // Remaining budget for one public native dispatch. These fields must stay
    // before the host-specific SPR storage/callback extension below so their
    // offsets agree with every CPUState implementation.
