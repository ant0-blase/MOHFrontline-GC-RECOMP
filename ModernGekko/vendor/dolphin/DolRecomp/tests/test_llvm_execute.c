...
static u32 external_read_count;
static u64 external_read_value;
static int external_read_bad;
...
static u64 external_read(CPUState* cpu, u32 ea, u8 size) {
    (void)cpu;
    external_read_bad |= ea != 0xCC00500Au || size != 4;
    external_read_count++;
    return external_read_value;
}
...
static void prepare_call(CPUState* cpu, u32 address) {
    ...
    cpu->native_cycle_budget = 256;
    cpu->native_guard_budget = 2048;
}
...
int main(void) {
    ...
    // Runtime helper/fallback boundaries preserve the same native quantum.
    prepare_call(&cpu, 0x80002C00u);
    cpu.gpr[3] = 0;
    cpu.downcount = 0;
    fallback_count = 0;
    func_80002C00(&cpu);
    CHECK(cpu.gpr[3] > 0 && cpu.gpr[3] < 1000);
    CHECK(cpu.pc == 0x80002C00u);
    CHECK(cpu.native_cycle_budget <= 0);
    CHECK(cpu.downcount < 0 && cpu.downcount > -1024);
    CHECK(fallback_count == cpu.gpr[3]);

    // The first dispatch polls an unchanged external value and yields before
    // re-reading the guarded instruction. CoreTiming is represented here by
    // the harness changing the value between public dispatches.
    prepare_call(&cpu, 0x80002D00u);
    cpu.gpr[3] = 0xCC00500Au;
    cpu.external_read = external_read;
    cpu.native_cycle_budget = 16;
    external_read_count = 0;
    external_read_value = 0;
    external_read_bad = 0;
    func_80002D00(&cpu);
    const u32 reads_before_event = external_read_count;
    CHECK(reads_before_event > 0 && reads_before_event < 32);
    CHECK(cpu.pc == 0x80002D00u);
    external_read_value = 1;
    cpu.native_cycle_budget = 16;
    cpu.native_guard_budget = 2048;
    func_80002D00(&cpu);
    CHECK(external_read_count == reads_before_event + 1);
    CHECK(!external_read_bad);
    CHECK(cpu.gpr[4] == 1);
    CHECK(cpu.pc == 0x81234564u);

    // A cycle whose closing edge exists only in the indirect LR switch is
    // still bounded, and exits at the continuation before executing it twice.
    prepare_call(&cpu, 0x80002E00u);
    cpu.gpr[3] = 0;
    cpu.native_cycle_budget = 12;
    func_80002E00(&cpu);
    CHECK(cpu.gpr[3] > 0 && cpu.gpr[3] < 32);
    CHECK(cpu.pc == 0x80002E04u);
    CHECK(cpu.native_cycle_budget <= 0);

    // Known generated cross-chunk target: side exit, no recursive call.
    prepare_call(&cpu, 0x80003100u);
    cpu.gpr[3] = 0;
    cpu.downcount = 0;
    func_80003100(&cpu);
    CHECK(cpu.pc == 0x80003200u);
    CHECK(cpu.gpr[3] == 0);
    CHECK(cpu.downcount == -1);
