...
public:
  ...
  // Relaxed, non-RMW variant for advisory flags where losing a concurrent
  // "set" only delays a future sleep opportunity and cannot affect correctness.
  // This deliberately avoids a locked cmpxchg on x86.
  bool TestAndClearRelaxed()
  {
    if (!m_val.load(std::memory_order_relaxed))
      return false;
    m_val.store(false, std::memory_order_relaxed);
    return true;
  }
