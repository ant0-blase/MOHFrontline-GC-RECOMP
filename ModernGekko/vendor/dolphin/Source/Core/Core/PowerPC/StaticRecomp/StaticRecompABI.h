...
typedef struct StaticRecompModuleDesc
{
  ...
  // ABI v5: optional runtime-hash selector for overlapping executable images.
  // The module may use this to select which statically recompiled image owns
  // a canonical chunk after a loader overwrites the same guest address range.
  int (*select_chunk_variant)(u32 chunk_index, u64 runtime_hash);
