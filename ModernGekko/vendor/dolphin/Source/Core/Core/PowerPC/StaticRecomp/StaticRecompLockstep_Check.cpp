...
void StaticRecompLockstepVerifier::LockstepCheck(u32 entry_pc, u32 end_pc, const CPUState& entry_state)
{
  ...
  while (steps < m_ls_step_cap)
  {
    ...
    // end_pc may legitimately be visited several times before the native
    // burst actually returns to the dispatcher (loops, call return addresses,
    // cyclic CFGs).  Match the native burst by guest work consumed, not by
    // the first occurrence of its endpoint.
