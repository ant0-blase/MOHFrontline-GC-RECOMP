...
void StaticRecompCore::VerifyChunk(u32 index)
{
  ...
  if (m_module->select_chunk_variant)
  {
    // ABI v5: the module owns the accepted-hash set for this canonical chunk.
    // This is what permits two runtime ELF/DOL images to reuse the same guest
    // addresses without weakening SMC validation: only a byte-for-byte hash
    // known at module build time is accepted.
    hash_verified = m_module->select_chunk_variant(index, hash) != 0;
    if (hash_verified)
  ...
  else
  {
    if (m_effective_chunk_hashes[index] == 0)
      m_effective_chunk_hashes[index] = hash;
    hash_verified = hash == m_effective_chunk_hashes[index];
  }
