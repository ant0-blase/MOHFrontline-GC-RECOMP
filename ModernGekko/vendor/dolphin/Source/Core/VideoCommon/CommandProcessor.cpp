...
void CommandProcessorManager::GatherPipeBursted()
{
  ...
  {
    // CPWritePointer is written only by the CPU/command-processor side.
    // Avoid an unnecessary locked RMW on every 32-byte gather-pipe burst;
    // the GPU only observes the published value.
    const u32 write_ptr = m_fifo.CPWritePointer.load(std::memory_order_relaxed);
    m_fifo.CPWritePointer.store(write_ptr + GPFifo::GATHER_PIPE_SIZE,
                                std::memory_order_relaxed);
  }
...
void CommandProcessorManager::SetCPStatusFromGPU()
{
  ...
  if (bp_enabled)
  {
    breakpoint = m_fifo.bFF_Breakpoint.load(std::memory_order_relaxed) != 0;
    const u32 read_pointer = m_fifo.CPReadPointer.load(std::memory_order_relaxed);
    if (m_fifo.CPBreakpoint.load(std::memory_order_relaxed) == read_pointer)
    {
      if (!breakpoint)
      {
